...
def add_company_user_to_db(company_id, user_info, contacts):
    ...
    try:
        ...
        # Insert contacts (company email will be fixed from frontend)
        for contact in contacts:
            contact_id = str(uuid.uuid4())
            c.execute(
                """
...
def log_alert_endpoint():
    ...
    if latitude is not None and longitude is not None:
        print(f"[LOCATION] Alert detected at lat={latitude}, lng={longitude}")
    else:
    ...
    try:
        ...
        VALUES (?, ?, CURRENT_TIMESTAMP, ?, ?, ?)""",
                    (
                        user_id,  # using user_id as session_id for now (hackathon-safe)
                        alert_type,  # alert_id
                        ALERT_THRESHOLD,  # count at trigger time
                        float(latitude),
                        float(longitude),
                    ),
                )
