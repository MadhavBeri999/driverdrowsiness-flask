# app_with_calls.py
# ─────────────────────────────────────────────────────────────────
# 🚨 MODIFIED version of app.py with Twilio emergency calling added.
#
# Changes vs app.py:
#   1. Imports twilio_utils (call_all_contacts)
#   2. get_contacts_for_user_with_phone() — fetches phone too
#   3. /log_alert also calls emergency contacts after sending emails
#   4. Response JSON includes "calls_made" count
#
# ✅ Original app.py is completely untouched.
# 🗑️  Delete this file + twilio_utils.py if you ever want to roll back.
#
# Run with: python app_with_calls.py
# ─────────────────────────────────────────────────────────────────
...
from twilio_utils import call_all_contacts  # ← NEW
import sqlite3
import uuid
import os
import time
...
# ------------------- Get User + Contacts (email only — unchanged) -------------------
...
# ── NEW: also fetches phone so Twilio can call ─────────────────────────────────
def get_contacts_for_user_with_phone(user_id: str):
    """
    Returns (driver_name, contacts_list) where each contact dict
    includes name, email, AND phone.
    """
    conn = get_db_connection()
    c = conn.cursor()
    try:
        ...
        driver_name = user_row["full_name"]
        c.execute(
            "SELECT name, email, phone FROM contacts WHERE user_id = ?", (user_id,)
        )
        contacts = [
...
# ------------------- Making the company tables  -------------------
def add_company_user_to_db(company_id, user_info, contacts):
    conn = get_db_connection()
    c = conn.cursor()
    ...
    try:
        c.execute(
            ...
            INSERT INTO company_users (
                id, company_id, full_name, age, email, phone, vehicle_number
            )
            VALUES (?, ?, ?, ?, ?, ?, ?)
        """,
            (
        ...
        for contact in contacts:
            ...
            c.execute(
                ...
                INSERT INTO company_contacts (
                    id, company_user_id, name, relation, email, phone
                )
                VALUES (?, ?, ?, ?, ?, ?)
            """,
                (
...
_last_notification_time = {}  # cooldown dict
...
def add_user():
    ...
    try:
        ...
        print(f"[app_with_calls.py] ✅ Latest active user set to {latest_user_id}")
...
def log_alert_endpoint():
    ...
    if latitude is not None and longitude is not None:
        print(f"[LOCATION] Alert at lat={latitude}, lng={longitude}")
    else:
        print("[LOCATION] No location received with alert")
    ...
    try:
        ...
        if triggered:
            driver_name = get_driver_name(user_id)
            ...
            try:
                ...
                db_cursor.execute(
                    """INSERT INTO alerts (session_id, alert_id, timestamp, count, latitude, longitude)
        VALUES (?, ?, CURRENT_TIMESTAMP, ?, ?, ?)""",
                ...
                db_conn.commit()
                db_conn.close()
                print("[DB] Alert with location stored successfully")
            except Exception as db_error:
                print("[DB ERROR] Failed to store alert location:", db_error)
            ...
            if now_ts - last_sent < COOL_DOWN_SECONDS:
                print(f"⏱️ Cooldown active for {key}. Skipping emails & calls.")
                return (
                    jsonify(
                        {
            ...
            # ── Fetch contacts (email + phone) ──────────────────
            driver_name, contacts = get_contacts_for_user_with_phone(user_id)
            if not contacts:
                print(f"⚠️ No contacts found for user {user_id}")
                return (
                    jsonify(
                        {"threshold_exceeded": True, "emails_sent": 0, "calls_made": 0}
                    ),
                    200,
                )
            ...
            # ── 📞 NEW: Place Twilio calls ──────────────────────
            calls_made = call_all_contacts(contacts, driver_name, alert_type)
            print(f"📞 Calls placed: {calls_made}")
                ...
                jsonify(
                    {
                        ...
                        "calls_made": calls_made,  # ← NEW field in response
                    }
                ),
                200,
            )
...
if __name__ == "__main__":
    print("[app_with_calls] Starting Flask app with Twilio calling. DB:", DB_PATH)
